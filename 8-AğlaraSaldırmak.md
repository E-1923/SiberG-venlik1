...
## 📌 1. WEP'in Özellikleri

- **RC4 Şifreleme Algoritması Kullanır:**
  - WEP, veri paketlerini şifrelemek için RC4 (Rivest Cipher 4) algoritmasını kullanır.
...
- **Statik Anahtar Kullanımı:**
  - Tüm istemciler aynı şifreleme anahtarını paylaşır, bu da saldırılara karşı savunmasız hale getirir.

📌 **En büyük zayıflığı:** IV (Initialization Vector) rastgele olmadığı için tekrar eden şifreleme desenleri oluşturur.

## 📌 2. WEP'in Zayıflıkları ve Kırılması
...
2. **Ağları tara:**
   ...
   ```
3. **Hedef ağı seç ve paket topla:**
   ```bash
   sudo airodump-ng -c [Kanal] --bssid [Ağ MAC] -w dump wlan0mon
   ```
4. **Paketleri analiz ederek WEP anahtarını kır:**
   ```bash
...
## 📌 3. WEP Yerine Hangi Protokoller Kullanılmalı?
...
🚨 WEP, artık güvensiz kabul edilir ve kesinlikle kullanılmamalıdır.
🚨 WPA2 veya WPA3 gibi daha güvenli protokoller tercih edilmelidir.
🚨 Eski ağ cihazları hala WEP kullanıyorsa, acilen güncellenmelidir.

# ⚠️ WEP Şifrelerini Kırma ve Güvenlik Açıkları

## 🔍 1. WEP Güvenlik Açıkları Neden Kırılabilir?
...
- Çok sayıda paket toplayarak WEP anahtarını analiz etmek mümkün olur.

## 🛠️ 2. WEP Şifresini Kırma Adımları
### 🖥️ Gerekli Araçlar
...
## 🔒 3. WEP Saldırılarına Karşı Korunma
...
1. **İstemci (Client)** bağlanmaya çalışır.
2. **Router**, kimlik doğrulama için rastgele bir "challenge" (şifreleme anahtarı) gönderir.
3. **İstemci**, şifreleme anahtarını doğrular ve cevap yollar.
4. **Router**, bağlantıyı onaylar ve istemci artık ağa bağlıdır.
...
- ✅ **Monitor Mode** destekleyen Wi-Fi adaptörü *(Örn: Alfa AWUS036NHA)*
...
```bash
sudo aireplay-ng -0 10 -a [BSSID] -c [İSTEMCİ_MAC] wlan0mon
```
📌 **Örnek:**
```bash
...
```
💡 **Wordlist** olarak **"rockyou.txt"** veya özel olarak hazırlanmış listeler kullanılabilir.
...
1. **Wi-Fi kartını "Monitor Mode"a al.**
2. **Hedef ağı belirle.**
3. **İstemciyi ağdan düşürerek tekrar bağlanmasını sağla.**
4. **Handshake paketlerini kaydet.**
5. **Şifreyi kırmak için wordlist kullan.**


```markdown
# WPA/WPA2 Handshake Yakalama ve Brute Force ile Şifre Kırma

## 1️⃣ WPA/WPA2 Handshake Yakalama

Kablosuz ağların parola doğrulama sürecini ele geçirerek çalışan "Handshake Yakalama" yöntemi aşağıdaki adımları içerir:

### 🔹 Adım 1: Monitor Modunu Açma
Wi-Fi adaptörünü Monitor Mode'a almak için aşağıdaki komutu kullanın:
```bash
sudo airmon-ng start wlan0
```
...
Bazı modemlerde "WPS PIN" güvenlik açığı bulunur.
🛠 Kullanılan Araç: **Reaver**
...
🛠 Kullanılan Araçlar: **hcxdumptool ve Hashcat**
...
## 1. Hazır Wordlist Kaynakları

### RockYou.txt
En popüler wordlistlerden biridir. Kali Linux içinde bulunur:
```bash
...
```bash
crunch 8 8 -t abc%%%%% -o pattern_list.txt
```
...
## 3. Online Wordlist Üreticileri

- 🌐 **Weakpass Generator** → Farklı kategorilerde şifre listeleri sağlar.
- 🌐 **WPA-PSK Wordlist Generator** → WPA2 şifreleri için özelleştirilmiş listeler oluşturur.
- 🌐 **Probable Wordlists** → Kullanıcı adı ve parola kombinasyonlarına dayalı listeler.
...
✅ **Online araçlar:** Weakpass, WPA-PSK Generator
